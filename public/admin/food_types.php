...
if (isset($_POST['add'])) {
    ...
    if (!empty($name)) {
        ...
        $stmt->execute([$name]);
        $_SESSION['message'] = "Food type added.";
...
if (isset($_POST['edit'])) {
    ...
    $name = trim($_POST['name']);
    if (!empty($name)) {
        $stmt = $conn->prepare("UPDATE food_types SET name = ? WHERE food_type_id = ?");
        $stmt->execute([$name, $id]);
        $_SESSION['message'] = "Food type updated.";
        header("Location: food_types.php");
        exit;
    }
}
if (isset($_GET['edit'])) {
    $eid = (int)$_GET['edit'];
    $r = $conn->prepare("SELECT * FROM food_types WHERE food_type_id = ?");
    $r->execute([$eid]);
    $editing = $r->fetch();
}
...
$params = [];
if ($search !== '') {
    $where .= ' AND ft.name LIKE ?';
    $params[] = '%' . $search . '%';
}
$sort_map = ['name' => 'ft.name', 'id' => 'ft.food_type_id', 'regions' => 'region_count'];
$order_col = $sort_map[$sort] ?? 'ft.name';

$result = $conn->prepare("SELECT ft.*, (SELECT COUNT(*) FROM regions WHERE food_type_id = ft.food_type_id) AS region_count FROM food_types ft WHERE $where ORDER BY $order_col $order");
...
.form-inline input[type=text]{padding:10px 14px;border-radius:8px;border:1px solid var(--border-color);color:var(--text-main);font-size:14px;outline:none;flex:1;min-width:200px;}
...
.btn-edit{background:var(--primary);color:var(--bg-dim);}
.btn-edit:hover{background:var(--primary-hover);}
.btn-del{background:rgba(204,36,29,0.15);color:var(--danger-hover);border:1px solid rgba(204,36,29,0.3);}
.btn-del:hover{background:rgba(251,73,52,0.25);}
.btn-cancel{background:transparent;color:var(--text-muted);border:1px solid var(--border-color);}
.btn-cancel:hover{border-color:var(--border-hover);color:var(--text-main);}
...
<div class="container">
<h2><?= $editing ? 'Edit' : 'Add'; ?> Food Type</h2>
...
<form method="POST" class="form-inline">
    ...
    <?php endif; ?>
    <input type="text" name="name" required placeholder="Food type name" value="<?= $editing ? htmlspecialchars($editing['name']) : ''; ?>">
    <button type="submit" name="<?= $editing ? 'edit' : 'add'; ?>" class="btn btn-add"><?= $editing ? 'Update' : 'Add'; ?></button>
    <?php if ($editing): ?>
        <a href="food_types.php" class="btn btn-cancel">Cancel</a>
    <?php endif; ?>
</form>

<h2>All Food Types</h2>

<form method="GET" class="filter-bar">
    <div class="fg">
        <label>Search</label>
        <input type="text" name="search" placeholder="Search food types..." value="<?= htmlspecialchars($search); ?>">
    </div>
    <div class="fg">
        <label>Sort</label>
        <select name="sort">
            ...
            <option value="regions" <?= $sort === 'regions' ? 'selected' : ''; ?>>Regions</option>
        </select>
    </div>
    ...
    <a href="food_types.php" class="btn-clear">Clear</a>
</form>

<table>
    <thead><tr><th>ID</th><th>Name</th><th>Regions</th><th>Actions</th></tr></thead>
    <tbody>
        <?php while ($ft = $result->fetch()): ?>
        <tr>
            <td><?= $ft['food_type_id']; ?></td>
            <td><strong style="color:#458589;"><?= htmlspecialchars($ft['name']); ?></strong></td>
